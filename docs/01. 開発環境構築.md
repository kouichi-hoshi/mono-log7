...
- 基本コマンド（ローカルのみ / 本番では実行しない）:
	...
	- Drift が出た場合のリセット（データ全消し）: `pnpm exec prisma migrate reset --force`
- CI / 本番（Neon）では `prisma migrate deploy` を実行（適用のみ）。Secretsは docs/07, docs/vercel-neon-setup.md 参照。
