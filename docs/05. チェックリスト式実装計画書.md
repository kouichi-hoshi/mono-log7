...
| ☐ | P2-APP-01 | 外観変更（ライト/ダーク/システム・文字サイズ） | テーマ切替UI（system/light/dark）でページ全体の見た目が切り替わり、リロード後も維持される（永続化）。`system` は OS 設定に追従。必要なら主要画面の回帰確認（トースト/モーダル含む）。テストでGREEN | P1-ENV-05 | - | - |
